...
def test_convert_1_file():
    ...
    print(file_1.parent)
    os.system(
        f"python rdfx.py convert -f {output_format} -o {str(file_1.parent)} {str(file_1)} "
    )
...
def test_convert_2_file():
    ...
    os.chdir("../rdfx")
    os.system(
        f"python rdfx.py convert -f {output_format} -o {str(file_1.parent)} {str(file_1)} {str(file_2)}"
    )
    file_1.with_suffix(f".{output_format}").unlink()
